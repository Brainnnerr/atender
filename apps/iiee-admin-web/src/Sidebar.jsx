      ...
      <aside style={{ 
        ...
        <div>
            ...
            {!collapsed ? (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
                  <img src="/IIEE-BG.png" alt="IIEE Logo" style={{ width: '34px', height: '34px', borderRadius: '50%', border: '1.5px solid #fde047', flexShrink: 0, objectFit: 'cover' }} />
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: '900', color: '#854d0e' }}>ATENDER</div>
                    <div style={{ fontSize: '9px', fontWeight: '800', color: '#64748b', letterSpacing: '0.5px' }}>IIEE CONTROL</div>
                  </div>
                </div>
              ...
              <button onClick={() => setCollapsed(false)} title="Expand Sidebar" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
                <img src="/IIEE-BG.png" alt="IIEE Logo" style={{ width: '36px', height: '36px', borderRadius: '50%', border: '1.5px solid #fde047', objectFit: 'cover' }} />
              </button>
            )}
          </div>
          ...
          <nav style={{ padding: '16px 10px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {menuItems.map(item => {
              const active = currentTab === item.id || (item.id === 'reports' && (currentTab === 'audit' || currentTab === 'reports-audits')) || (item.id === 'summary' && currentTab === 'student-summary') || (item.id === 'fines' && currentTab === 'fine-management');
              return (
                <button 
                  key={item.id}
      ...
      {showConfirmModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, fontFamily: 'sans-serif' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '32px', borderRadius: '20px', width: '380px', textAlign: 'center', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize: '13px', color: '#64748b', margin: '0 0 24px 0', lineHeight: '1.4' }}>Are you sure you want to sign out from the IIEE Admin Console?</p>
