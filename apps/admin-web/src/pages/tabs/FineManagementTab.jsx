...
export default function FineManagementTab({ currentUser }) {
  ...
  .select('id, amount, status, remarks, student_id, event_id, events(semester)');
      ...
      // 3. Aggregate fines per student correctly separating unpaid vs paid balances
      const aggregatedList = students.map(student => {
        const studentFines = finesData.filter(f => f.student_id === student.id);
        ...
        if (unpaidAmount === 0 && hasPaidRecords) {
          status = 'paid';
          // Find original accumulated amount from paid records if available, or default
  ...
  const uniqueSemesters = ['ALL', ...new Set(fines.flatMap(f => f.semester || '1st Semester'))];
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      doc.save(`Student_Fines_Audit_Report_${new Date().toISOString().slice(0, 10)}.pdf`);

      await logAdminAction({
        currentUser,
        actionType: 'EXPORT_AUDIT_REPORT',
        module: 'FINES',
        details: { export_format: 'PDF', total_records: filteredFines.length },
      });
    } catch (err) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 print:hidden">
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          <h3 className="text-base font-black text-slate-800">Total Unpaid Fines</h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          <h3 className="text-base font-black text-slate-800">Total Collected (Paid)</h3>
        ...
        <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 space-y-4 text-left">
            ...
            <div className="space-y-2 py-2">
              {['Member', 'Athlete', "Dean's Lister", 'Officer', 'Others'].map((rem) => (
