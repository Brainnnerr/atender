  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (error) throw error;
        showToast(`PICE Event "${title}" created successfully!`);
      }
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          <div style={{ fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total PICE Assemblies</div>
          <div style={{ fontSize: '28px', fontWeight: '700', color: '#0f172a', marginTop: '6px' }}>{totalEvents}</div>
          <div style={{ fontSize: '11px', color: '#94a3b8', fontWeight: '500', marginTop: '4px' }}>Isolated Chapter Ledger</div>
        </div>
        ...
        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
          ...
          <div style={{ fontSize: '11px', color: '#16a34a', fontWeight: '500', marginTop: '4px' }}>Accepting BSCE Check-Ins</div>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h2 style={{ fontSize: '16px', fontWeight: '700', textTransform: 'uppercase', color: '#0f172a', margin: 0, letterSpacing: '0.5px' }}>PICE Events & Attendance Management</h2>
          <p style={{ fontSize: '12px', color: '#64748b', marginTop: '4px', fontWeight: '400' }}>
            Publish chapter assemblies, fines, and geofenced QR codes independent of FCO.
          </p>
        </div>
        <button
            ...
            {['ALL', 'OPEN', 'CLOSED'].map((st) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  backgroundColor: statusFilter === st ? '#4a0404' : 'transparent',
                ...
                {filteredEvents.length === 0 ? (
                  <tr>
                    <td colSpan="6" style={{ padding: '48px', textAlign: 'center', color: '#94a3b8', fontWeight: '600', textTransform: 'uppercase' }}>
                      No PICE events found matching your criteria.
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={evt.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: '14px 16px' }}>
                          <button
                            ...
                            style={{ backgroundColor: '#fffbeb', color: '#b45309', border: '1px solid #fde68a', padding: '6px 12px', borderRadius: '8px', fontWeight: '600', fontSize: '11px', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}
                          >
                            <QrCode size={13} />
                            <span>QR Stand</span>
                          </button>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '24px', maxWidth: '420px', width: '100%', padding: '32px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '13px', fontWeight: '700', textTransform: 'uppercase', color: '#0f172a', margin: 0, letterSpacing: '0.5px' }}>PICE Event QR Stand</h3>
            ...
            <div ref={printRef} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                <img src="/PICE-LOGO.jpg" alt="PICE Logo" style={{ width: '38px', height: '38px', borderRadius: '50%', border: '2px solid #b45309', objectFit: 'cover' }} />
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: '11px', fontWeight: '700', color: '#b45309' }}>PICE ESSU CHAPTER</div>
              ...
              <div style={{ padding: '16px', backgroundColor: '#ffffff', borderRadius: '16px', border: '2px dashed #b45309', marginBottom: '20px' }}>
                <QRCodeSVG
                  value={JSON.stringify({
                    eventId: activeQrEvent.id,
                    title: activeQrEvent.title,
                    organizationId: PICE_ORG_ID,
                    type: 'PICE_EVENT_ATTENDANCE',
                  })}
                  size={190}
                  level="H"
                  includeMargin={true}
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button
                ...
                style={{ flex: 1, backgroundColor: '#4a0404', color: '#ffffff', border: 'none', padding: '10px', borderRadius: '10px', fontWeight: '600', fontSize: '12px', textTransform: 'uppercase', cursor: 'pointer' }}
              >
                Print Stand
              </button>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                <h3 style={{ fontSize: '15px', fontWeight: '700', textTransform: 'uppercase', color: '#0f172a', margin: 0, letterSpacing: '0.5px' }}>
                  {isEditing ? 'Update PICE Event' : 'Create PICE Event'}
                </h3>
                <p style={{ fontSize: '11px', color: '#64748b', marginTop: '2px', fontWeight: '400' }}>Scoped strictly to PICE organization ledger.</p>
              </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g., PICE Structural Seminar"
                  value={title}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g., ESSU Engineering Building"
                  value={location}
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '10px' }}>
                ...
                <button type="submit" disabled={submitting} style={{ flex: 1, backgroundColor: '#4a0404', color: '#ffffff', border: 'none', padding: '10px', borderRadius: '10px', fontWeight: '600', fontSize: '12px', textTransform: 'uppercase', cursor: 'pointer', opacity: submitting ? 0.5 : 1 }}>{submitting ? 'Saving...' : 'Save Event'}</button>
              </div>
            </form>
          </div>
        </div>
